...
|---------|------------|-------------------------------------------------|------------|-------|
| 1.0.8   | 2026-09-01 | `.pkg` built + signed, handed to Transporter (first drop rejected with a 409 on `BlackmagicRawAPI.framework/Versions/A/Libraries/DecoderMetal` — nested framework Mach-O kept Blackmagic's Developer ID sig; `mac_sign_and_package.sh` gained a Phase 3b that re-signs nested framework Mach-O with the 3rd Party Mac Developer cert; re-packaged and re-submitted) | built, notarized (submission `bc878be7-f1a8-47e1-9f3c-2638e08a4a01`, Accepted), stapled, queued in Store Manager ingest `Apps/cineflow/1.0.8` (pending_review as of this entry) | Big feature run: BRAW/R3D proxy decode fixes + `BlackmagicRawAPI.framework` now bundled in `Contents/Frameworks/`; new ARRI ART (`art-cmd`) decode provider for ARRIRAW / ARRIRAW MXF (no Resolve needed); RED `--format 201` ProRes fix; Camera Match Lab results layout aligned + de-verbosed; multi-LUT preview; Media Review waveform/audio/LUT fixes; first-run decoder-deps notice; Sony X-OCN research (no free CLI, stays Resolve). `deploy_direct_macos.sh` also gained nested-framework signing. |
| 1.0.7   | 2026-08-17 | uploaded via Transporter, build created successfully | built, notarized, queued in Store Manager (pending_review as of this log entry) | App icon fix only — regenerated `icon.icns`/`icon.png`/`icon.ico` with proper Big Sur+ canvas margin (icon previously filled the canvas edge-to-edge with no inset, causing a grey backplate/undersized-logo look in the Dock; new margin ~8.5%, matched against Mail/Terminal/VS Code). Bumped straight past `1.0.6` per this log's own rule even though `1.0.6`'s App Store Connect review outcome is unknown, since App Store Connect's version-uniqueness check is keyed to upload, not release status. |
